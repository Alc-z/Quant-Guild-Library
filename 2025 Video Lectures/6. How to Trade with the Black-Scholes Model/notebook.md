...
专门用于期权市场
用 股票 + 无风险债券 复制一个期权的 payoff，然后说"期权的合理价格就是复制它的成本"。


> 不是用于价格预测，而是用于衡量\
> 将比较尺度统一为 隐含波动率IV，通过比较近远期波动率大小，来描述市场
...
前四个（股价、行权价、剩余期限、无风险利率）都是可观测的。只有波动率 (\sigma) 是未知的。
市场上期权的交易价格 (C_{\text{market}}) 已经摆在那里了——那是所有买方和卖方真金白银博弈出来的结果。
所以实战中，我们做的是：

把 $(S, K, T, r, C_{\text{market}})$ 代进去，反解出 $\sigma$，得到 隐含波动率 (Implied Volatility, IV)。

这个 IV 不是一个"历史统计值"，而是市场当下对未来波动的一致预期。
...
Black-Scholes 模型的核心假设是标的**资产价格**遵循 **几何布朗运动 (Geometric Brownian Motion)**：
...
# 打印权利金与盈亏
print("Premium at t=0:", black_scholes_call(100, 100, .3, .05, 1)) # S=100, K=100, sigma=30%, r=5%, t=1year
...
$\Delta_{call} = \Phi(d_1)$ ，代入本例参数（$S=100, K=100, r=0.05, \sigma=0.3, t=1 year$）：
$$d_1 = \frac{\ln(100/100) + (0.05 + 0.3^2/2) \times 1}{0.3 \times \sqrt{1}} = \frac{0 + 0.095}{0.3} \approx 0.3167$$

查标准正态累积分布函数：$\Delta_{call} = \Phi (d1) = \Phi(0.3167) \approx 0.624$

公式中另一项 $\Phi(d_2)$ 是风险中性测度下期权到期实值（ITM）的概率：
$$d_2 = d_1 - \sigma \sqrt{t} = 0.3167 - 0.3 \times 1 = 0.0167$$
$$\Phi(d_2) = \Phi(0.0167) \approx 0.507$$

即在此参数下，模型认为该期权有约 **50.7%** 的概率实值到期。

模拟显示：
- 初始 Delta 为 0.624（平值期权），股价上涨 $1$ 块，看涨期权价格大约上涨 $0.624$。
- 随着模拟路径数量增加，实值到期的比例趋向于 $\Phi(d_2) \approx 0.507$（大数定律）
...
> 💡 ATM IV和现价约金额和Delta都是TWS内显示的
> 💡 还不熟悉 "30 Delta" 这种说法的含义？参见 [附录：解释期权行权 →](#解释期权行权)
...
> 其中 `13.43 @ 14.10` 是标准的做市商双边报价格式：
>
> | 价格 | 含义 |
> |------|------|
> | **13.43** | **Bid（买价）**——做市商愿意从你手里买入这张期权的价格 |
> | **@** | 分隔符 |
> | **14.10** | **Ask/Offer（卖价）**——做市商愿意卖给你这张期权的价格 |
>
> 所以：
> - 你想**买** → 付 **$14.10**（Ask）
> - 你想**卖** → 收 **$13.43**（Bid）
> - 差价 $14.10 - $13.43 = **$0.67**（Spread），这是做市商赚的"过路费"
...
"30 Delta" 是交易员用 Delta 来"选合约"的行话。
...
|Delta|	含义|
|---|---|
|~50 (0.50)|	平值 (ATM)，行权价 ≈ 现价|
|30 (0.30)|	虚值看涨 (OTM call)，行权价高于现价|
|15 (0.15)	|更虚值|
|70+ (0.70+)|	实值 (ITM)|


"交易 30 Delta" = 选一张 Delta ≈ 0.30 的看涨期权来交易。这不是一个数学定义，而是一个合约选择惯例——交易员说"给我 30 delta 的 call"，Broker 就知道要找哪个行权价的合约。

这个"行权价高出股价多远"取决于两个因素：
- **隐含波动率越高** → 30 Delta 的行权价离现价越远（市场预期波动大，需要更大的价差才能达到相同的 Delta）
- **剩余时间越长** → 30 Delta 的行权价离现价越远（时间多，不确定性大）

所以同样是 30 Delta：
- NVDA（$210, 高 IV）→ 行权价约 $222.5，高出 **$12.5**
- GOOGL（$346, 低 IV）→ 行权价约 $365，高出 **$20**

虽然 Nvidia 的价差绝对值更小，但占股价的比例（约 6%）比 Google（约 5.5%）更高，因为 Nvidia 的隐含波动率更高。

**为什么用 Delta 而不是行权价？**

因为不同股票的股价量级不同（NVDA \$210 vs GOOGL \$346），固定说"买 \$222.5 的 call"只对 Nvidia 有意义。
但说"买30 Delta call"，不管股价是 $10 还是 $1000，它都表示同一个意思——一个虚值约 1 个标准差左右的合约，是一个标准化的选合约方式。

**和之前讨论 $\Phi(d_2) \approx 0.507$ 的区别：**

$\Phi(d_2)$ 是模型算出来的理论概率（本例 ≈ 50.7%）
"30 Delta" 说的是 Delta = 0.30，不是概率——这里 Delta 只是被当做定位标尺来用，不是真的认为有 30% 的概率实值到期（第 5.3 节已经解释了现实市场里不能这么理解）