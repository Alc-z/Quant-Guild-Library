...
## 引入非平稳性(Non-Stationarity)：Edge 的衰减
...
均值回复过程（Ornstein-Uhlenbeck 过程）是指参数围绕某个长期均值波动的过程，常用于量化、利率、价差、波动率建模：
...
其中：
- $X_t$ = 时刻的目标变量（价差、利率、波动率、价格残差等）
- $\theta$ = 均值回复速度，$\theta$ 越大，偏离后回归均值越快；$\theta \to 0$ 几乎不回复，趋近随机游走。
- $\mu$ = 长期均值
- $\sigma$ = 波动率
- $dW_t$ = 标准布朗运动增量
